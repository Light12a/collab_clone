...
class CallerId(Base):
    ...
    project_id = Column(BIGINT(20), ForeignKey(Project.project_id), nullable=False)
...
class CallerIdGroup(Base):
    ...
    tenant_id = Column(String(256), ForeignKey(Tenant.tenant_id), nullable=False)
    caller_num_id = Column(BIGINT(20), ForeignKey(CallerId.caller_num_id), nullable=False)
    group_id = Column(String(256), ForeignKey(Group.group_id), nullable=False)
    insert_date = Column(DateTime, nullable=False, server_default=text("current_timestamp()"))
    update_date = Column(DateTime, nullable=False, server_default=text("current_timestamp()"))
...
class CallerIdUser(Base):
    ...
    user = relationship(Group, backref='caller_id_user')