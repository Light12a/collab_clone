...
class User(Base):
   ...
   locked = Column(TINYINT(1),server_default = text("0"), comment='Lock State : 0 is not locked, 1 is locked')
...
class MaintenanceUsers(Base):
   ...
   __table_args__ = {"comment":"Maintenance User : Manage information of manager user"}
...
class LoginHistories(Base):
   __tablename__ = 'login_histories'
   __table_args__ = {"comment":"Login Histories : Manage information of user action login"}
   id = Column(BIGINT(20), primary_key = True, nullable = False)
   user_id = Column(ForeignKey(User.user_id), nullable = False)
   login_date = Column(DateTime,server_default=text("current_timestamp()"))
