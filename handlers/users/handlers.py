...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if data:
            ...
            if lock:
                ...
                raise gen.Return(
                    self.error(code=HTTPStatus.UNAUTHORIZED.value, message="User is locked"))
            ...
            if check:
                ...
                if not credential:
                    self.write_response(
                        401, code=HTTPStatus.UNAUTHORIZED.value,
    ...
    def _check_token_valid(self, request):
        ...
        try:
            if result[0][0]:
                self.write_response(
                    200,code=HTTPStatus.OK.value, response_data={"code":200})
        except IndexError:
            resp = {
                "code": 401,
            ...
            self.write_response(
                401,code=HTTPStatus.UNAUTHORIZED.value, response_data=resp)
    ...
    def post(self):
        ...
        if data:
            ...
            if check:
                ...
                self.write_response(
                    200, code=HTTPStatus.OK.value)
                log.info("User logged out successfully with token: {}".format(data['token']))
            else:
                self.write_response(
                    401, code=HTTPStatus.UNAUTHORIZED.value, message="Token is wrong")
        else:
            log.error("Request json is wrong")
            raise gen.Return(
                self.error(code=HTTPStatus.BAD_REQUEST.value, message="Bad request"))
