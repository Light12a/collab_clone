...
from .schema import LOGIN_SCHEMA, LOGOUT_SCHEMA, REFRESH_SCHEMA, PROFILE_SCHEMA, LOGOUT_CPM_SCHEMA
...
import asyncio
class LogoutCPMHandler(BaseHandler):
   SCHEMA = LOGOUT_CPM_SCHEMA
   ...
   def post(self):
      ...
      if self.get_current_user() is None:
         raise gen.Return(self.unauthorized())
      else:
         query = self.db.query(User).join(Token, User.id == Token.user_id).filter(
             Token.token_id == self.get_current_user())
         user = self.to_json(query[0])
         
         self.db.query(Token).filter(Token.token_id ==
                                     self.get_current_user()).delete()
         self.db.commit()
         self.clear_cookie('user')
         # raise gen.Return(self.write_response(
         #     "0000", code=HTTPStatus.OK, message="Logout successfully"))
         asyncio.create_task(self._task(UserId))

   def core_settings(self, events):
      return

   @gen.coroutine
   def _task(self, user):
      ami = AMIClient(host='13.113.23.145', port=5038,
                         username='vagrant', secret='vagrant')
      ami.create_action(
             {'action': 'Hangup', "channel": "/^PJSIP/webphone1-.*$"
             }, self.core_settings)
      yield ami.connect()