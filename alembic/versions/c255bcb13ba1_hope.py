...
Revision ID: c255bcb13ba1
Revises: 
Create Date: 2022-03-18 10:58:14.081489
...
def upgrade():
    ...
    op.create_table('away_reason',
    sa.Column('id', mysql.BIGINT(display_width=20), nullable=False, comment='Id'),
    sa.Column('tenant_id', sa.String(length=256), nullable=True, comment='Tenant Id : Tenant Id which these away reasons are belong to'),
    sa.Column('away_reason', sa.String(length=1024), nullable=True, comment='Away Reason : Content of Away reason'),
    sa.Column('is_display', mysql.TINYINT(display_width=1), nullable=True, comment='Is Display : 0 is not displayed, 1 is displayed'),
    sa.Column('update_date', sa.DateTime(), nullable=True, comment='Update date : Last time that away reason is changed'),
    sa.ForeignKeyConstraint(['tenant_id'], ['tenant.tenant_id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id'),
    comment='Away Reason : Away Reason'
    )
    op.create_index(op.f('ix_away_reason_tenant_id'), 'away_reason', ['tenant_id'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_away_reason_tenant_id'), table_name='away_reason')
    op.drop_table('away_reason')
